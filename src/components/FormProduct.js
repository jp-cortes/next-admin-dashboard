...
function checkData(data) {  // REGEX
    let pass = true;
    if (!data.title.match(/\w{5,}[a-zA-Z]/g)) {
      alert('Title must be only letters and at least 5 characters');
      pass = false;
    } if (!(data.price).toString().match(/^[0-9]+$/g)) {
      alert('Invalid price');
      pass = false;
    } if (!data.description.match(/\w{5,}[a-zA-Z]/g)) {
      alert('Description must be at least 5 characters and letters');
      pass = false;
    } if (!data.images[0].match(/^.+\.(jpg|jpeg|png)$/g)) { //verify the extesion
